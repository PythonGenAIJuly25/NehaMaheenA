...
--explanation: We assign a group number by subtracting ROW_NUMBER() from each log_id, 
--so consecutive IDs get the same group. Then we group by this value to find the MIN and MAX (start and end) of each consecutive sequence.
...
--explanation: We use DENSE_RANK() to rank employees within each department by highest salary.
--Then, we filter (WHERE rnk = 1) to get the top earner(s) from each department and join with the Department table to get department names.
...
-- explanation:We first find the maximum salary in each department using GROUP BY departmentId.
--Then, we select employees whose (departmentId, salary) matches the maximum, 
--and join with the Department table to show their names and departments.
...
-- explanation:This query groups all calls between unique pairs of people (ignoring self-calls) and 
--calculates the total number of calls and total duration for each pair.
--It ensures each pair appears only once using LEAST() and GREATEST() functions and sorts the results show tables ;
